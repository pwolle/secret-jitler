...
def kill_player(
	...
	key: shtypes.random_key) -> shtypes.killed:

	'''
	Function for the whole execution-process:
		- checks if players can be shot
		- if legal, killable (iE neither dead nor president) play is shot
	...
	number = jrn.choice(key, jnp.arange(player_number),shape=(1,), p=probabilities)
...
def executive_full(
	...
	player_number:shtypes.player_num,
	probabilities: jtp.Float[jtp.Array, "player_num"], 
	key: shtypes.random_key) -> tuple[shtypes.winner,shtypes.killed]:
